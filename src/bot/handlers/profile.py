...
def show_user_data(message, bot):
    ...
    if not profile.user:
        bot.send_message(message.chat.id, utils.t(profile, "Siz tizimda emassiz.", "Вы не в системе."))
        return
    ...
    if response and response.status_code == 200:
        ...
        bot.send_message(message.chat.id,
                         utils.t(profile, "❌ Ma'lumotlarni yuklashda xatolik yuz berdi. Iltimos, qayta urinib ko'ring.",
...
def handle_become_entrepreneur(message, bot):
    ...
    if response and response.status_code == 200:
        ...
        is_now_entrepreneur = user_data.get('is_entrepreneur', False)

        profile.is_entrepreneur = is_now_entrepreneur
